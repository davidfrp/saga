...
export class GitService {
  ...
  // TODO how is this different from doesPullRequestExist?
  ...
  async checkoutBranch(
    branch: string,
    options?: { remote?: string; track?: boolean },
  ): Promise<void> {
    const flags = []
  ...
  async getPopularBaseBranches(): Promise<string[]> {
    ...
    const branches = pullRequests.map((pullRequest: PullRequestResponse) => ({
      name: pullRequest.baseRefName,
      date: new Date(pullRequest.mergedAt || pullRequest.updatedAt),
    }))
