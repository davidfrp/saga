...
import { Issue, StatusCategory, Transition } from "../../services/jira/types.js"
import { AuthenticatedCommand } from "../../legacy/legacyAuthenticatedCommand.js"
import {
  askAssignYou,
  askBranch,
...
import { GitService } from "../../legacy/legacyGitService.js"
import { JiraService } from "../../services/jira/JiraService.js"
import { Task, TaskStatus, Tasker } from "../../legacy/legacyTasker.js"
...
export default class LegacyStart extends AuthenticatedCommand {
  ...
  async run(): Promise<void> {
    ...
    const { args, flags } = await this.parse(LegacyStart)
    ...
    if (remoteBranchMatches.length > 0) {
      ...
      if (shouldCheckOutRemoteBranch) {
        await git.checkoutBranch(remoteBranch, { track: true })
        return this.exit(0)
      }
    }
    ...
    const branch = await askBranch(defaultBranchName, (value: string) => {
    ...
    if (await git.doesOpenPrExist(branch, prBaseBranch)) {
      console.error(
        `${chalk.red("✗")} ${format(
          "An open pull request already exists for %s into %s", // A pull request for %s into %s already exists.
          chalk.cyan(branch),
          chalk.cyan(prBaseBranch),
        )}`,
      )
    ...
    const tasks: Task<TaskerContext>[] = [
      {
        ...
        action: async () => {
          ...
          await validateIsCurrentBranch(branch)
